...
**Açıklama:** `diagrams/ERDiagram.png`'deki şemanın (EMPLOYEE, DEPARTMENT, ROLE/PERMISSION, VEHICLE/RESERVATION, SHUTTLE_ROUTE/STOP, MENU/MEAL/MENU_ITEM, SURVEY/SURVEY_QUESTION/SURVEY_RESPONSE/FEEDBACK, ANNOUNCEMENT/NOTIFICATION, CHAT__, POLICY__) Flyway migration dosyalarına (`V1__init.sql`, vb.) dönüştürülmesi. Docker Compose ile yerel PostgreSQL 16 + pgvector + pg_trgm extension kurulumu. `weekly_schedule` alanı ER diyagramına göre `EMPLOYEE` tablosunda `jsonb` olarak tutulur; ayrı `WEEKLY_SCHEDULE`/`SCHEDULE_DAY` tablosu yoktur.
