...
// Tek oturumluk sohbet: mesajlar yalnizca yerel state'te tutulur, sayfa
// yenilenince kaybolur (session gecmisi kavrami yok). A-8.
type ChatMessage = {
  id: string;
  role: "user" | "assistant";
  text: string;
      ...
      <div ref={scrollRef} className="flex-1 space-y-4 overflow-auto pr-1">
        {messages.length === 0 && !mutation.isPending && (
          <p className="text-muted-foreground text-sm">
            Şirketle ilgili bir soru sorun; asistan yanıtlasın.
          </p>
        )}
        {messages.map((m) => (
